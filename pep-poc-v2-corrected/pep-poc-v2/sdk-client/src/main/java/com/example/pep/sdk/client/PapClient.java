...
public final class PapClient {
    ...
    public PapClient(RestClient restClient, Retry retry, CircuitBreaker circuitBreaker) {
        this.restClient = restClient;
        this.retry = retry;
        this.circuitBreaker = circuitBreaker;
    }

    /** Returns the PAP response body on success; throws {@link PapException} on rejection/unavailability. */
    public String send(PapRequest request) {
        Supplier<String> call = () -> doSend(request);
    ...
    private String doSend(PapRequest request) {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);

        // Catalog-declared HEADER attributes
