...
/**
 * Demonstrates the three source-annotation types and their precedence
 * (@PapProperty > @PapAttribute > @PapInclude):
 *
 *  - "propagate", "resource_type_id"  → @PapProperty (class constants, no entity column)
 *  - "id", "code", "name", "description" → @PapAttribute (entity state)
 *  - "tenant_id" → @PapInclude (pulled from the Tenant relationship: pipeline.tenant.id)
 */
@Entity
@PapEntity(
...
public class Pipeline {
    ...
    public Pipeline(String code, String name, String description, Tenant tenant) {
        this.code = code;
        this.name = name;
